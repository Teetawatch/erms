<x-app-layout>
    <x-slot name="header">แก้ไขฝ่าย: {{ $department->name }}</x-slot>

    <div class="max-w-lg">
        <form action="{{ route('admin.departments.update', $department) }}" method="POST" class="card p-6 space-y-4">
            @csrf @method('PUT')
            <div>
                <label class="block text-sm text-erms-muted mb-1">ชื่อฝ่าย *</label>
            ...
            <div class="flex items-center gap-3 pt-2">
                <button type="submit" class="btn-primary">อัปเดตฝ่าย</button>
